...
export const LocationProvider = (props) => { // not all machinery in the warehouse is available to use in different areas of the warehouse
    const [locations, setLocation] = useState([]) // the sole purpose of useState is to establish an application state variable. it gives us a variable (first item in parenthsis) it gives us a function to change the state (second item in parenthesis)

    const getLocations = () => {
        return fetch("http://localhost:8088/locations?_expand")
            .then(res => res.json())
            .then(setLocation)
